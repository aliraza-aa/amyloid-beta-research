...
class BlockAnalysis:
    ...
    def __init__(self, x, multi=1, weights=None, bias=None, T=None, interval_low=None, interval_up=None, dt=1):
        ...
        if self.w is None:
            ...
            self.kbT = 0.008314463*T
            self.w /= self.w.sum()
            self.stat = fblocking(self.x, self.w, self.kbT,
                                  self.multi, self.interval)
    ...
    def SEM(self):
        ...
        for i, b in enumerate(self.stat):
            ...
            c[i] -= minimize(fun=find_n_intersect, x0=b[1],
                             args=self.stat[self.stat[..., 0] > b[0]], bounds=bnds).fun
    ...
    def get_pdf(self, cv=None):
        ...
        if cv is not None:
            ...
            u = gaussian_kde(cv, bw_method="silverman",
                             weights=self.w).evaluate(x)
        else:
            u = gaussian_kde(self.x, bw_method="silverman",
                             weights=self.w).evaluate(x)
        ...
        for n in range(1, Nb+1):
            ...
            pdf_i = gaussian_kde(
                self.x[start:end], bw_method="silverman", weights=self.w[start:end]).evaluate(x)
